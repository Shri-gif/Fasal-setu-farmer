...
const POPULAR_PRESETS = [
  {
    name: 'Fresh Desi Tomato (देसी टमाटर)',
    cat: 'cat-veg',
    unit: 'kg',
    price: 35,
  ...
  {
    name: 'Pure Mustard Oil / Seeds (सरसों)',
    cat: 'cat-spice',
    unit: 'litre',
    price: 160,
    img: 'https://images.unsplash.com/photo-1471193945509-9ad0617afabf?auto=format&fit=crop&w=600&q=80',
  },
];
...
export const AddProductView: React.FC<AddProductViewProps> = ({
  ...
  /*
    ...
    price_per_unit always remains the farmer's original/base price.
  ...
  */
  const [price, setPrice] = useState<string>(
    existingProduct ? String(existingProduct.price_per_unit) : ''
  );

  const [unit, setUnit] = useState(
    existingProduct?.unit || 'kg'
  );
  ...
  const [harvestDate, setHarvestDate] = useState(
    existingProduct?.harvest_date ||
    new Date().toISOString().split('T')[0]
  );

  const [farmLocation, setFarmLocation] = useState(
    existingProduct?.farm_location ||
    defaultFarmLocation ||
  ...
  // ---------------------------------------------------------
  // ISOLATED PLATFORM FEE STATE
  // ---------------------------------------------------------

  const [platformSettings, setPlatformSettings] =
    useState<PlatformFeeSettings | null>(null);

  const [isPlatformFeeLoading, setIsPlatformFeeLoading] =
    useState(true);
  ...
  useEffect(() => {
    if (existingProduct) {
      ...
      /*
        Existing product price is the farmer base price.
        No fee is reversed here.
      */
      setPrice(
        String(existingProduct.price_per_unit || '')
      );
      ...
      setStock(
        String(existingProduct.stock ?? 0)
      );
  ...
  const handleApplyPreset = (
    preset: typeof POPULAR_PRESETS[0]
  ) => {
    setName(preset.name);
    ...
    /*
      Preset price is also the farmer's base price.
    */
    setPrice(
      String(preset.price)
    );
  ...
  const handleSubmit = async (
    ...
    if (!name.trim()) {
      setFormError(
        'Please enter the crop / product name.'
      );
    ...
    if (!categoryId) {
      setFormError(
        'Please select a valid crop category.'
      );
    ...
    if (
      ...
      setFormError(
        'Please provide a valid price per unit (greater than 0).'
      );
    ...
    try {
      /*
        ...
        We save ONLY the farmer's base price.

        DO NOT save:
        priceBreakdown.customerPrice
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              {/* ================================================= */}
              {/* ISOLATED PLATFORM FEE PREVIEW */}
              {/* ================================================= */}

              <div className="mt-2 rounded-xl bg-emerald-50 border border-emerald-200 px-3 py-2.5">
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between mt-1 text-xs">
                      ...
                      <span className="text-slate-600">
                        ...
                        {priceBreakdown.feeType ===
                        'percentage'
                          ? `(${priceBreakdown.feeValue}%)`
                          : '(fixed)'}
