...
import { CUSTOMER_PORTAL_URL } from '../lib/supabase';
...
export const CustomerControlView: React.FC<CustomerControlViewProps> = ({
  ...
  const { orders, customers, updateOrderStatus, deleteOrder, exportData, isLiveConnected } = useData();
  ...
  return (
    <div className="space-y-6" id="customer-control-view">
      {/* Top Header & Customer Portal Connection Banner */}
          ...
          <div className="flex items-start space-x-3.5">
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs text-slate-400 font-mono">https://shri-gif.github.io/Fasal-setu-customer-/</span>
              </div>
      ...
      {/* Main Tab Selector (Orders vs Registered Customers) */}
      <div className="flex items-center space-x-2 border-b border-slate-800 pb-2">
        ...
        >
          ...
          <span>Registered Buyers ({customers.length})</span>
        </button>
      </div>

      {activeMainTab === 'orders' ? (
        <>
          {/* Toolbar & Pipeline Filter Tabs */}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex flex-wrap items-center gap-1.5 p-1 bg-[#0c1017] rounded-2xl border border-slate-800">
                {(
                  ...
                  ] as { id: 'all' | OrderStatus; label: string; count: number }[]
                ).map((tab) => (
                  <button
                    key={tab.id}
                  ...
                  >
                    ...
                    <span
                      className={`px-1.5 py-0.2 rounded-full text-[10px] ${
                        statusTab === tab.id ? 'bg-slate-950 text-white' : 'bg-slate-800 text-slate-400 border border-slate-700'
                      }`}
                    >
                      {tab.count}
                    </span>
                ...
                <div className="flex items-center space-x-1 border-l border-slate-800 pl-2">
                  <button
                    ...
                    className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold ${
                      viewMode === 'table' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    Table
                  </button>
                  <button
                    ...
                    className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold ${
                      viewMode === 'kanban' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    Kanban
                  </button>
          ...
          {viewMode === 'table' ? (
                      ...
                      <tr>
                        <td colSpan={7} className="py-16 text-center text-slate-500">
                          <div className="flex flex-col items-center justify-center space-y-2">
                            ...
                            <p className="text-sm font-semibold text-slate-400">No real customer orders recorded in Supabase yet.</p>
                            ...
                            >
                              ...
                              <span>Create First Supabase Order</span>
                            </button>
                          </div>
                        </td>
                      </tr>
                        ...
                        return (
                          <tr key={order.id} className="hover:bg-[#161c28]/60 transition-colors">
                            ...
                            <td className="py-3.5 px-4">
                              <p className="font-mono font-bold text-slate-200">#{order.id}</p>
                            ...
                            {/* Produce Details */}
                            <td className="py-3.5 px-4">
                              <p className="font-medium text-slate-200">{order.product_name || 'Farm Produce'}</p>
                              <p className="text-[10px] text-slate-500">
                                {order.quantity} units @ ₹{order.price_per_unit}/unit
                              </p>
                            </td>

                            {/* Delivery Location */}
                            <td className="py-3.5 px-4">
                              <p className="text-slate-300">{order.city || 'Lucknow'}</p>
                            ...
                            {/* Amount */}
                            <td className="py-3.5 px-4">
                              <p className="font-bold text-emerald-400 text-sm">₹{order.total_amount}</p>
                              <p className="text-[10px] text-slate-400">
                                Produce: ₹{order.subtotal_amount || order.quantity * order.price_per_unit} + Fee: ₹{order.platform_fee_amount || Math.round((order.total_amount || 0) * 0.10)}
                              </p>
                            </td>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {(['pending', 'confirmed', 'dispatched', 'delivered'] as OrderStatus[]).map((stage) => {
                const stageOrders = orders.filter((o) => o.status === stage);
                return (
                  <div key={stage} className="bg-[#11151c] rounded-3xl p-4 border border-slate-800 shadow-lg shadow-black/20 flex flex-col">
                    <div className="flex items-center justify-between pb-3 border-b border-slate-800 mb-3">
                      <div className="flex items-center space-x-2">
                        <span className={`h-2.5 w-2.5 rounded-full ${
                          stage === 'pending' ? 'bg-yellow-400' :
                          stage === 'confirmed' ? 'bg-amber-400' :
                          stage === 'dispatched' ? 'bg-blue-400' : 'bg-emerald-400'
                        }`} />
                    ...
                    <div className="space-y-2.5 overflow-y-auto max-h-[500px] flex-1">
                      {stageOrders.length === 0 ? (
                        <p className="text-[11px] text-slate-600 text-center py-6">No orders in {stage}</p>
                      ) : (
                        stageOrders.map((ord) => (
                          <div
                            key={ord.id}
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <span className="font-mono font-bold text-xs text-slate-200">#{ord.id}</span>
                              <span className="font-bold text-xs text-emerald-400">₹{ord.total_amount}</span>
                            </div>
                            <div>
                              <p className="text-xs font-semibold text-white">{ord.customer_name}</p>
                              <p className="text-[10px] text-slate-400">{ord.product_name || 'Farm Produce'}</p>
                            </div>
                            <div className="flex items-center justify-between text-[10px] text-slate-500 pt-1 border-t border-slate-900">
                              <span>{ord.city}</span>
                              <span>{ord.quantity} units</span>
                            </div>
                          </div>
                        ))
                      )}
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-[#0c1017] border-b border-slate-800 text-slate-400 font-bold">
                  <tr>
                    ...
                    <th className="py-3.5 px-4">Gross Purchases</th>
                    <th className="py-3.5 px-4">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800">
                  {filteredCustomers.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-16 text-center text-slate-500">
                        <div className="flex flex-col items-center justify-center space-y-2">
                          ...
                          <p className="text-sm font-semibold text-slate-400">No registered buyers in Supabase yet.</p>
                          <p className="text-xs text-slate-500 max-w-md">
                    ...
                    filteredCustomers.map((cust) => {
                      ...
                      const waLink = cleanPhone ? `https://wa.me/${cleanPhone.startsWith('91') ? cleanPhone : '91' + cleanPhone}` : '#';

                      return (
                        <tr key={cust.id} className="hover:bg-[#161c28]/60 transition-colors">
                          <td className="py-3.5 px-4">
                            <div className="flex items-center space-x-2.5">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-slate-500">ID: {cust.id.slice(0, 8)}...</p>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="py-3.5 px-4 text-slate-300">
                            <p>{cust.city || cust.state || 'India'}</p>
                            {cust.delivery_address && (
                              <p className="text-[10px] text-slate-500 truncate max-w-[150px]">{cust.delivery_address}</p>
                            )}
                          </td>

                          <td className="py-3.5 px-4 font-bold text-white">
                            {cust.total_orders_count || 0} orders
                          </td>
